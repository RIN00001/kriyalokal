...
<div data-kriya-product="{{ $product['id'] }}"
    ...
    <a href="{{ $href }}" class="relative overflow-hidden" style="aspect-ratio: 4/5; display:block; background: var(--color-kriya-brown-deep);">
        ...
        <div class="absolute bottom-3 left-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            @if ($canBuyBackend)
                <form method="POST" action="{{ route('cart.store', $backendId) }}">
                    @csrf
                    <button type="submit"
                        class="w-full rounded-full text-xs font-bold py-2 text-center"
        ...
        <div class="mt-auto flex items-center justify-between pt-3"
            ...
            @if ($canBuyBackend)
                <form method="POST" action="{{ route('cart.store', $backendId) }}">
                    @csrf
                    <button type="submit"
                        class="inline-flex items-center gap-1.5 rounded-full text-[11px] font-bold px-3.5 py-1.5 transition"
